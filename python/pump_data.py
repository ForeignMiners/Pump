...
#        candle["Close"] = float(kline[0][4])
        ...
        prices = []
        f = open("../data/"+s+".csv","a")
        f.write("Symbol,Timestamp,Date,Open,High,Low,Close,Volume,MA_High,MA_Volume,P_Anomaly,V_Anomaly\n")
        f.close()
        for k in kline:
            vol = float(k[5])
            price = float(k[2])
            ...
            f = open("../data/"+s+".csv","a")
            f.write(str(s)+","+str(k[0])+","+str(datetime.fromtimestamp(date/1000))+","+str(k[1])+","+str(k[2])+","+str(k[3])+","+str(k[4])+","+str(k[5])+","+str(ma_price)+","+str(ma_vol)+","+str(price_anomaly)+","+str(vol_anomaly)+"\n")
            f.close()
